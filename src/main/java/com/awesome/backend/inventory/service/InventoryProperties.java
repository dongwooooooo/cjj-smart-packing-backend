 ...
 *
 * @param collector  스냅샷 집계기. 기본 5초 — 시연 화면 새로고침 주기와 같다. settleSeconds 는
 *                   정착 창: 이보다 최근에 기록된 원장 행은 아직 스냅샷에 접지 않는다(StockBalanceCollector)
...
public record InventoryProperties(@DefaultValue Collector collector, @DefaultValue Reconciler reconciler) {
    ...
    public record Collector(@DefaultValue("5000") long intervalMs, @DefaultValue("60") long settleSeconds) {
    }
